...
void mainmenu(){
	...
	text = "BRINDAVAN COLLEGE OF ENGINEERING";
	screen = worldToScreen(455 - 235, 500);
	drawBigTextonWindow(text.data(), text.size(), screen.x, screen.y);

	glColor3f(0,0,0);
	text = "Computer Graphics Mini Project on:";
	screen = worldToScreen(450 - 238, 385);
	drawTextonWindow(text.data(), text.size(), screen.x, screen.y);

	glColor3f(0,0,0);
	text = "\"DEADLOCK DETECTION AND RECOVERY\"";
	screen = worldToScreen(450 - 238, 360);
	drawBigTextonWindow(text.data(), text.size(), screen.x, screen.y);

	glColor3f(0,0,0);
	text = "DONE BY:";
	screen = worldToScreen(420, 260);
	drawTextonWindow(text.data(), text.size(), screen.x, screen.y);

	glColor3f(0,0,0);
	text = "AHMED NOOR";
	screen = worldToScreen(410, 230);
	drawTextonWindow(text.data(), text.size(), screen.x, screen.y);
	glColor3f(0,0,0);
	text = "(1BO15CS002)";
	screen = worldToScreen(401, 210);
	drawTextonWindow(text.data(), text.size(), screen.x, screen.y);
